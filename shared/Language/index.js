...
class Language extends Component {
  ...
  _handleOnSearchTextChange = value => {
    ...
    var countryData = [];
    var tempCountryData = [];
    countryData = LanguageData.filter(country => {
      return (
        country.name.toLowerCase().startsWith(value) ||
        country.nativeName.toLowerCase().startsWith(value)
      );
    });
    tempCountryData = countryData;
    LanguageData.filter(country => {
    ...
    this.setState({LanguageData: countryData});
  };
  _handleClearSearch = () => {
  ...
  _handleSetCountry = (name, nativeName) => {
    global.Country.getCountry(name, nativeName);
    this._handleGoBack();
  };
  render() {
    return (
      <View style={styles.viewFlagsListContainer}>
        ...
        <View style={styles.viewFlagTitleContainer}>
          ...
          <View style={styles.viewFlagTitle}>
            <Text style={styles.textFlagTitle}>Select Language</Text>
          </View>
        ...
        <View>
          <FlatList
            ...
            renderItem={({item}) => (
              <TouchableWithoutFeedback
                onPress={() =>
                  this._handleSetCountry(item.name, item.nativeName)
                }>
                <View style={styles.countryStyle}>
                  ...
                  <View style={styles.viewCountryFlag}>
                    <Text style={styles.textFontSize}>{item.nativeName}</Text>
                  </View>
